...
PEM_FILE="/home/pi/Footstep/config/scripts/reverse-ssh/footstep-wsn.pem"
...
su $SUDO_USER -c "ssh -i $PEM_FILE $MIDDLEMAN_USERNAME@$MIDDLEMAN_SERVER \"echo I am in\""

echo "Checking to see if GatewayPorts is set on $MIDDLEMAN_SERVER"
su $SUDO_USER -c "ssh -i $PEM_FILE $MIDDLEMAN_USERNAME@$MIDDLEMAN_SERVER \"cat /etc/ssh/sshd_config | grep 'GatewayPorts yes'\""
